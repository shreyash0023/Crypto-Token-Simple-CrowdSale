...
contract GBTokenAndCrowdsale {
    ...
    address tokenFundsAddress; // Calling of contract and its address
    
    /* Approved address of the account that will receive the raised Ether funds */
    address beneficiary;  // The person who created the contract 
    
    /* Keep track of ETH funds raised */
    uint amountRaised;

    bool saleIsFinished = false;
    ...
    */
    ...
    function sendTokens(address receiver, uint amount) public {
        ...
        if (balances[tokenFundsAddress] < amount) return;
        
        // take funds out of sender's account
        balances[tokenFundsAddress] -= amount;
    ...
    function withdrawRaisedFunds() public {
        ...
        if (msg.sender != beneficiary && saleIsFinished==true)
            return;
